...
const Footer = () => {
  ...
  const phoneNumber = "081336920647"; // Ganti dengan nomor telepon yang ingin disalin
  ...
  return (
    <div className={`${style.paddingX} max-w-[1300px] mx-auto flex flex-col`}>
      ...
      <div className="flex flex-col ss:flex-row-reverse ss:justify-between mb-8">
        <div
          className={`${style.paragraph} flex flex-col xs:flex-row xs:justify-between`}
        >
          <ul className="mb-4 ss:mb-0 ss:mr-1 sm:mr-6 md:mr-10">
            {infos.map((info) => {
              if (info.id !== "whatsapp") {
                return (
                  <li className="mb-4 w-fit" key={info.id}>
                    <a
                      href={info.link}
                      target="blank"
          ...
          <div className="flex flex-col mb-12 ss:mb-0">
            ...
            {isCopy && (
              <p className="text-base mb-1 bg-primary w-fit py-1 px-2">
                Nomer tercopy!
              </p>
            )}
            <p
            ...
            >
              <div className="w-8">{setIcon(infos[infos.length - 1].id)}</div>
              <p className="ml-2">{infos[infos.length - 1].content}</p>
            </p>
            <a href={getWaApi(true)} target="blank" className={style.btnYellow}>
              Hubungi Sekarang
            </a>
          </div>
        </div>
        <div className="w-fit ss:w-44 sm:w-fit flex flex-col justify-between">
          <a href="/">
            <img
              ...
              src="https://rfidnusantara.com/images/2024-03-1.png"
              alt=""
            />
          </a>
          <p className={style.paragraph}>
            &copy; {currentYear} RFID Nusantara All rights reserved.
          </p>
        </div>
      </div>
    </div>
