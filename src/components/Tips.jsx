...
const Tips = () => {
  ...
  return (
    <div className={`${style.paddingX} w-full mb-20`} id="service">
      <div className="max-w-[1300px] m-auto flex flex-col items-center">
        <h1 ref={startAnimating} className={`${style.heading2} my-10 sm:my-16`}>
          Layanan Kami
        </h1>
        <div className="flex flex-col sl:flex-row items-center justify-between">
          {enjoy.map((tips) => (
            ...
            >
              <img
                src={tips.img}
                className="mb-5   sl:mb-12"
                alt="Enjoy Loka Chips"
              />
              <h3 className={style.heading3}>{tips.title}</h3>
              <p className={`w-11/12 ${style.paragraph} mb-12`}>
                {tips.content}
              </p>
            </div>
          ))}
